  ...
  return (
    <tr id={id} ref={setNodeRef} style={style} {...attributes} {...listeners}>
      {mapIndexed((columnHeader: string, index: number) => {
        return <td key={index}>{cellValueGetter(rowValue, columnHeader)}</td>;
      })(columnHeaders)}
    </tr>
  );
}
...
export default function TableWithDraggableRows<T>({
  cellValueGetter,
  columnHeaders,
  rowsObject,
  itemIDs,
  setItemIDs,
}: TableWithDraggableRowsProps<T>) {
    ...
    >
      <SortableContext items={itemIDs} strategy={rectSwappingStrategy}>
        <Table bordered hover>
          <thead>
            <tr>
              {mapIndexed((header: string, key: number) => (
                <th key={key}>{header}</th>
              ))(columnHeaders)}
            </tr>
          </thead>
          <tbody>
            {mapIndexed((rowID: string, rowIndex: number) => (
              <DraggableTableRow
